...
public class Controller
{
     ...
     throws GeneratorException
  {
    ...
    Option opt = unitConfiguration.getOptions().getGlobalScope().get(new QualifiedName("dumpxml"));
    if(opt != null && "true".equals(opt.getValue()) && modelRoot instanceof SourceElement && source.getSourceFile() != null)
    {
      dumpXml(source, (SourceElement) modelRoot);
    }
  ...
  private void dumpXml(Source source, SourceElement modelRoot)
     throws GeneratorException
  {
    String input = source.getSourceFile().getName();

    File output = new File("/tmp/dump.xml");
    output.delete();

    try(FileWriter fw = new FileWriter(output))
    {
      fw.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
      modelRoot.xmlDump(fw);
      fw.write("\n");
    }
